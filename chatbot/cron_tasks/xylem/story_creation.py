...
XYLEM_SESSION_TYPE = 'XylemX_entrepreneurship_development'
XYLEM_BOT_ROUTE = '/xylem_story_creation'
...
def chat_sessions_without_story(session_type=XYLEM_SESSION_TYPE):  
    """
...
def chat_session_ids_without_story(session_type=XYLEM_SESSION_TYPE):  
...
def create_story():
    try:
        company_bot = CompanyBot.objects.filter(route=XYLEM_BOT_ROUTE).first()
        if not company_bot:
            logger.error('No CompanyBot found for route=%s', XYLEM_BOT_ROUTE)
            return

        community_fgd_session_for_story = ChatSession.objects.filter(
            session=OuterRef('session'),
            session_type=XYLEM_SESSION_TYPE,
        )
