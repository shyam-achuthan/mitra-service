...
from chatbot.models import ChatSession, ChatStatus, CompanyBot, CompanyChat, Story
...
PPPI_SET_1_SESSION_TYPE = "PPPI_Set_1"
PPPI_SET_1_BOT_ROUTE = "/story-creation-pppi-set1"
...
def chat_sessions_without_story(session_type=PPPI_SET_1_SESSION_TYPE):
    """
...
def chat_session_ids_without_story(session_type=PPPI_SET_1_SESSION_TYPE):
    """`session` values (string ids) for ChatSessions that have no matching Story."""
    return chat_sessions_without_story(session_type=session_type).values_list(
        "session", flat=True
    )
...
def _generate_story_for_session(company_bot, session):
    ...
    if company_bot.provider not in (
        ...
        logger.warning(
            "Unsupported provider=%s for bot=%s", company_bot.provider, company_bot.id
        )
        return None
    ...
    for attempt in range(1, STORY_LLM_MAX_ATTEMPTS + 1):
        try:
            ...
            log_fn = (
                logger.error if attempt == STORY_LLM_MAX_ATTEMPTS else logger.warning
            )
            log_fn(
...
def _persist_story_from_llm_response(session_id, response):
    ...
    chat_session = (
        ChatSession.objects.filter(session=session_id).select_related("profile").first()
    )
...
def create_story():
    try:
        company_bot = CompanyBot.objects.filter(route=PPPI_SET_1_BOT_ROUTE).first()
        if not company_bot:
            logger.error("No CompanyBot found for route=%s", PPPI_SET_1_BOT_ROUTE)
            return

        pppi_set_1_session_for_story = ChatSession.objects.filter(
            session=OuterRef("session"),
            session_type=PPPI_SET_1_SESSION_TYPE,
        )
        matching_stories = Story.objects.filter(Exists(pppi_set_1_session_for_story))

        sessions_missing_story = chat_sessions_without_story()

        logger.info(
            "Creating story for PPPI Set 1 (%s stories, %s chat sessions without story)",
            matching_stories.count(),
            sessions_missing_story.count(),
        )
        for session in sessions_missing_story.values_list("session", flat=True):
            response = _generate_story_for_session(
                company_bot=company_bot, session=session
            )
            ...
            logger.info(
                ...
                story.id,
                session,
                PPPI_SET_1_BOT_ROUTE,
                story.title,
                list(story.other_params.keys()) if story.other_params else [],
            )

    except Exception as e:
        logger.error("Error creating story for PPPI Set 1: %s", e)
