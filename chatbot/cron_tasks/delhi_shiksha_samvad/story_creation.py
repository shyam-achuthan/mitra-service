...
DELHI_SHIKSHA_SAMVAD_SESSION_TYPE = 'delhi-shiksha-samvad'
DELHI_SHIKSHA_SAMVAD_BOT_ROUTE = '/story-creation-delhi-shikshasamvad'
...
def chat_sessions_without_story(session_type=DELHI_SHIKSHA_SAMVAD_SESSION_TYPE):
    """
...
def chat_session_ids_without_story(session_type=DELHI_SHIKSHA_SAMVAD_SESSION_TYPE):
...
def create_story():
    try:
        company_bot = CompanyBot.objects.filter(route=DELHI_SHIKSHA_SAMVAD_BOT_ROUTE).first()
        if not company_bot:
            logger.error('No CompanyBot found for route=%s', DELHI_SHIKSHA_SAMVAD_BOT_ROUTE)
            return
        ...
        delhi_session_for_story = ChatSession.objects.filter(
            ...
            session_type=DELHI_SHIKSHA_SAMVAD_SESSION_TYPE,
        )
        ...
        for session in sessions_missing_story.values_list('session', flat=True):
            ...
            logger.info(
                ...
                story.id,
                session,
                DELHI_SHIKSHA_SAMVAD_BOT_ROUTE,
                story.title,
