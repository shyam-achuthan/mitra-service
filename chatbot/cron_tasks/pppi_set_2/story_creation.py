...
PPPI_SET_2_SESSION_TYPE = 'PPPI_Set_2'
PPPI_SET_2_BOT_ROUTE = '/story-creation-pppi-set2'
...
def chat_sessions_without_story(session_type=PPPI_SET_2_SESSION_TYPE):
    """
...
def chat_session_ids_without_story(session_type=PPPI_SET_2_SESSION_TYPE):
...
def create_story():
    try:
        company_bot = CompanyBot.objects.filter(route=PPPI_SET_2_BOT_ROUTE).first()
        if not company_bot:
            logger.error('No CompanyBot found for route=%s', PPPI_SET_2_BOT_ROUTE)
            return

        pppi_set_2_session_for_story = ChatSession.objects.filter(
            session=OuterRef('session'),
            session_type=PPPI_SET_2_SESSION_TYPE,
        )
        matching_stories = Story.objects.filter(Exists(pppi_set_2_session_for_story))

        sessions_missing_story = chat_sessions_without_story()

        logger.info(
            'Creating story for PPPI Set 2 (%s stories, %s chat sessions without story)',
        ...
        for session in sessions_missing_story.values_list('session', flat=True):
            ...
            logger.info(
                ...
                story.id,
                session,
                PPPI_SET_2_BOT_ROUTE,
                story.title,
                list(story.other_params.keys()) if story.other_params else [],
            )

    except Exception as e:
        logger.error('Error creating story for PPPI Set 2: %s', e)