...
COMMUNITY_FGD_SESSION_TYPE = 'community-fgd'
COMMUNITY_FGD_BOT_ROUTE = '/story-creation-community-fgd'
...
def chat_sessions_without_story(session_type=COMMUNITY_FGD_SESSION_TYPE):  
    """
...
def chat_session_ids_without_story(session_type=COMMUNITY_FGD_SESSION_TYPE):  
...
def create_story():
    try:
        company_bot = CompanyBot.objects.filter(route=COMMUNITY_FGD_BOT_ROUTE).first()
        if not company_bot:
            logger.error('No CompanyBot found for route=%s', COMMUNITY_FGD_BOT_ROUTE)
            return

        community_fgd_session_for_story = ChatSession.objects.filter(
            session=OuterRef('session'),
            session_type=COMMUNITY_FGD_SESSION_TYPE,
        )
        matching_stories = Story.objects.filter(Exists(community_fgd_session_for_story))

        sessions_missing_story = chat_sessions_without_story()

        logger.info(
            'Creating story for Community FGD (%s stories, %s chat sessions without story)',
        ...
        for session in sessions_missing_story.values_list('session', flat=True):
            ...
            logger.info(
                ...
                story.id,
                session,
                COMMUNITY_FGD_BOT_ROUTE,
                story.title,
                list(story.other_params.keys()) if story.other_params else [],
            )

    except Exception as e:
        logger.error('Error creating story for Community FGD: %s', e)